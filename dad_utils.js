  ...
  import { serverState } from './dad_server_ctx.js';
...
////////// FETCH /////////////////////////////////
  ...
  async function serverFetch(uid,...args) {
      if(!uid) return console.error('DAD: provide uid to serverFetch');
      ...
      if(isServer){
        ...
        else console.error('DAD: stream write() not accessible');
        return resp;
      } 
        ...
        if(args[0][0]==='/') {
          ...
          while(!value && timeout<ssrfetchtimeout) { 
            ...
            timeout+=10; //ms
            if(timeout>ssrfetchtimeout) console.error('DAD: serverFetch timeout',uid);
          }
