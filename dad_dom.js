import {reactive, untrack} from './dad_dom_signal.js';
import { html } from './dad_html.js';
import {diffArrays} from './dad_dom_array.js';
export {render, renderClient, onMount, onUnmount, hydrate, renderAttribute};


import {createFragment, replaceFragments, clearFragment} from './dad_dom_fragments.js';
import {hydrateTree, hydrateAsyncTree} from './dad_dom_hydrate.js';
...
////////// RENDER ////////////////////////////////
  ...
  function renderClient(frag, t, owner={0:{}}){
    ...
    if(!frag) return console.error('DAD: renderClient missing node element');
    if(!frag.fragment) frag=createFragment(frag);

    if(typeof t === 'function' && !t.html) t=t();
    if(typeof t === 'function' && t.html) t=t();

    if(t.html===undefined) return console.error('DAD: unknown input to renderClient',t);
    ...
    for (let i=0; i<reactarray.length; i++) {
      ...
      switch(type) {
        ...
        case 'node':
            ...
            if(!placeholder) console.error('DAD: cannot find placeholder','rx'+key,root);
            else {
        ...
        case ':':
            ...
            if(!placeholder) console.error('DAD: cannot find attribute',key+i);
            else {
                ...
                else console.error('DAD: unknown special attr',type,i);
...
//////////////////////////////////////////////////
  ...
  async function hydrate(root, comp){
    if(!root) return console.error('DAD: hydrate missing root element');
    ...
    while(!el && timeout<ssrtimeout) {
      ...
      timeout+=10; //ms
      if(timeout>ssrtimeout) return console.error('DAD: hydrate timeout');
    }
    root=el;
