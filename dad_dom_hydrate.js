...
import {createFragment, updateFragment, extractFragmentFromDom} from './dad_dom_fragments.js'
import {renderAttribute} from './dad_dom.js'
...
////////// HYDRATE ///////////////////////////////
  ...
  async function hydrateTree(oldnode, newnode, _newfrag=false, idx=false){
      ...
      if(oldnode.nodeType===8&&oldnode.data==='cx') { //SSR tells us it's a CLIENT ONLY COMPONENT //oldnode.tagName==='CX'
        ...
        else console.error('DAD: unknown CX hydration case',oldnode,newnode)
      }
...
*/
        ...
        if(oldnode.outerHTML!==newnode.outerHTML && oldnode.data!=='cx') console.error(`DAD: hydration diff:\n\nSERVER:${oldnode.outerHTML}\n\nCLIENT:${newnode.outerHTML}`)          
  ...
  function hydrateAsyncTree(placeholder){
      ...
      if(idx===-1) {
        //DEGUGhyd&&console.error('DAD: error hydrating async',placeholder,asyncHydrates)
        return
      }
      else {
