...
////////// LITERALS TEMPLATE PARSER //////////////
  function parseAttribute(string, v, reactarray, newvalues){
      ...
      if(!ev) {console.error('DAD: attribute is missing :@ prefix',string); return false;}
      if(ev?.length>1) {console.error('DAD: attribute is missing "${}"',string); return false;}
      if(!string.endsWith(ev[0]+'"')) {console.error('DAD: attribute '+ev[0]+' is missing "${}"'); return false;}
      const tag = ev[0][0];
        ...
        for (let i=0; i<values.length; i++) {
          ...
          if(typeof v === 'function' || v instanceof Promise || v?.signal ) {
            if(!isAttribute) {              
              ...
              else console.error('DAD: unknown attribute type',strings[i],v);
            }
          } 
